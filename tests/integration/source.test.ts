...
before(async () => {
  ...
  const initial = (await observer.query('SELECT (SELECT count(*)::text FROM source.entities) AS entities, (SELECT count(*)::text FROM source.outbox) AS outbox, source_epoch::text FROM source.source_identity')).rows[0];
...
async function withWriter<T>(label: string, work: (writer: pg.Client) => Promise<T>): Promise<T> {
  const writer = await connect('writer', label);
  try { return await work(writer); } finally { await writer.end(); }
}
...
async function entity(id: string, client = observer) {
  return (await client.query('SELECT entity_id::text, source_epoch::text, entity_version::text, change_id::text, recorded_at::text, is_deleted, payload::text AS payload_json FROM source.entities WHERE entity_id=$1', [id])).rows;
}
async function revisions(id: string, client = observer) {
  return (await client.query('SELECT allocation_id::text, source_epoch::text, entity_id::text, entity_version::text, change_id::text, recorded_at::text, is_deleted, payload::text AS payload_json FROM source.outbox WHERE entity_id=$1 ORDER BY entity_version', [id])).rows;
}
async function sqlError(client: pg.Client, sql: string, code: string, params: unknown[] = []) {
  let observed = false;
  try { await client.query(sql, params); }
  catch (error) {
    observed = true;
    assert.equal((error as { code: string }).code, code, sql);
    evidence('SQL-rejection', { sql, expectedCode: code, actualCode: (error as { code: string }).code, message: (error as Error).message });
  }
...
test('T01 mutation lifecycle, exact BIGINT boundaries, no-op, and two committed revisions', { timeout: 20_000 }, async () => {
  {
    ...
    assert.deepEqual(history.map((r) => r.entity_version), ['1', '2', '3', '4']);
    assert.equal(new Set(history.map((r) => r.change_id)).size, 4);
    assert.equal(new Set(history.map((r) => r.source_epoch)).size, 1);
    const epoch = (await observer.query('SELECT source_epoch::text FROM source.source_identity')).rows[0].source_epoch;
    ...
    for (const [version, deleted, payload] of expected) {
      const result: pg.QueryResult<{ payload_matches: boolean; deletion_matches: boolean; time_matches: boolean }> = await observer.query('SELECT payload IS NOT DISTINCT FROM $3::jsonb AS payload_matches, is_deleted=$4 AS deletion_matches, recorded_at BETWEEN $5::timestamptz AND clock_timestamp() AS time_matches FROM source.outbox WHERE entity_id=$1 AND entity_version=$2', [created.entity_id, version, payload, deleted, floor]);
      assert.deepEqual(result.rows, [{ payload_matches: true, deletion_matches: true, time_matches: true }]);
    }
    const highVersion = await observer.query('SELECT source.next_version(9007199254740992) AS unsafe_number_boundary, source.next_version(9223372036854775806) AS maximum');
    assert.deepEqual(highVersion.rows, [{ unsafe_number_boundary: '9007199254740993', maximum: '9223372036854775807' }]);
    await sqlError(observer, 'SELECT source.next_version(9223372036854775807)', '22003');
    ...
    const duplicates = await observer.query('SELECT source_epoch, entity_id::text, entity_version::text FROM source.outbox GROUP BY source_epoch, entity_id, entity_version HAVING count(*) > 1');
...
test('T03 real narrowly scoped outbox INSERT error rolls back the source mutation', { timeout: 20_000 }, async () => {
  await withWriter('T03', async (writer) => {
    const original = await createEntity(writer, '{"state":"before-outbox-error"}');
    const beforeState = await entity(original.entity_id);
    const beforeEvents = await revisions(original.entity_id);
    ...
    try {
      await sqlError(writer, "SELECT * FROM source.mutate_entity($1,'update','{\"state\":\"must-rollback\"}'::jsonb)", 'P9001', [original.entity_id]);
      assert.deepEqual(await entity(original.entity_id), beforeState);
      assert.deepEqual(await revisions(original.entity_id), beforeEvents);
      evidence('T03', { beforeState, afterState: await entity(original.entity_id), beforeEvents, afterEvents: await revisions(original.entity_id), sqlState: 'P9001' });
    } finally { await observer.query('DROP TRIGGER test_reject_one_outbox ON source.outbox; DROP FUNCTION source.test_reject_one_outbox()'); }
    assert.equal((await observer.query("SELECT count(*)::text AS count FROM pg_trigger WHERE tgname='test_reject_one_outbox'")).rows[0].count, '0');
    await mutateEntity(writer, original.entity_id, 'update', '{"state":"after-cleanup"}');
    assert.deepEqual((await revisions(original.entity_id)).map((r) => r.entity_version), ['1', '2']);
  });
});

test('T04 concurrent writers wait on an observed row lock and create successive revisions', { timeout: 25_000 }, async () => {
  ...
  try {
    ...
    assert.deepEqual(history.map((r) => r.entity_version), ['1', '2', '3']);
    assert.deepEqual(history.map((r) => r.payload_json), ['{"writer": "initial"}', '{"writer": "A"}', '{"writer": "B"}']);
    assert.equal((await entity(original.entity_id))[0].payload_json, '{"writer": "B"}');
...
test('T05 historical after-images stay unchanged and evidence cannot be edited or deleted', { timeout: 20_000 }, async () => {
  await withWriter('T05', async (writer) => {
    const original = await createEntity(writer, '{"history":{"nested":[1,"first"]}}');
    await mutateEntity(writer, original.entity_id, 'update', '{"history":{"nested":[2,"second"]}}');
    const recorded = await revisions(original.entity_id);
    assert.deepEqual(recorded.map((r) => r.payload_json), ['{"history": {"nested": [1, "first"]}}', '{"history": {"nested": [2, "second"]}}']);
    await mutateEntity(writer, original.entity_id, 'delete');
    await mutateEntity(writer, original.entity_id, 'restore', '{"history":{"nested":[4,"restored"]}}');
    assert.deepEqual((await revisions(original.entity_id)).slice(0, 2), recorded);
    for (const sql of ['UPDATE source.outbox SET payload=\'{}\' WHERE entity_id=$1', 'DELETE FROM source.outbox WHERE entity_id=$1']) await sqlError(writer, sql, '42501', [original.entity_id]);
    await sqlError(observer, 'UPDATE source.outbox SET payload=\'{}\' WHERE entity_id=$1', '0A000', [original.entity_id]);
    await sqlError(observer, 'DELETE FROM source.outbox WHERE entity_id=$1', '0A000', [original.entity_id]);
    assert.deepEqual((await revisions(original.entity_id)).slice(0, 2), recorded);
    evidence('T05', { expectedHistoricalFields: recorded, afterLaterMutations: await revisions(original.entity_id) });
  });
});

test('T06 actual runtime login, owners, grants, protected metadata and unsupported SQL rejection', { timeout: 25_000 }, async () => {
  await withWriter('T06', async (writer) => {
    const identity = (await writer.query("SELECT session_user, current_user, pg_backend_pid() AS pid, rolsuper, rolcreaterole, rolcreatedb, rolreplication, rolbypassrls, pg_has_role(current_user, 'source_owner', 'MEMBER') AS owner_member FROM pg_roles WHERE rolname=current_user")).rows[0];
    assert.equal(identity.session_user, 'source_writer');
    assert.equal(identity.current_user, 'source_writer');
    for (const field of ['rolsuper', 'rolcreaterole', 'rolcreatedb', 'rolreplication', 'rolbypassrls', 'owner_member']) assert.equal(identity[field], false);
    const owners = (await observer.query("SELECT c.relname, pg_get_userbyid(c.relowner) AS owner FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='source' ORDER BY c.relname")).rows;
    assert.ok(owners.length >= 5);
    for (const object of owners) assert.equal(object.owner, 'source_owner');
    assert.equal((await observer.query("SELECT pg_get_userbyid(nspowner) AS owner FROM pg_namespace WHERE nspname='source'")).rows[0].owner, 'source_owner');
    const ownerRole = (await observer.query("SELECT rolcanlogin, rolsuper FROM pg_roles WHERE rolname='source_owner'")).rows[0];
    assert.deepEqual(ownerRole, { rolcanlogin: false, rolsuper: false });
    const functions = (await observer.query("SELECT p.proname, pg_get_userbyid(p.proowner) AS owner, p.prosecdef, p.proconfig, has_function_privilege('source_writer',p.oid,'EXECUTE') AS writer_execute, EXISTS(SELECT 1 FROM aclexplode(p.proacl) a WHERE a.grantee=0 AND a.privilege_type='EXECUTE') AS public_execute FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='source' ORDER BY p.proname")).rows;
    ...
    for (const fn of functions) {
      ...
      assert.equal(fn.writer_execute, ['create_entity', 'mutate_entity'].includes(fn.proname as string));
      assert.equal(fn.prosecdef, ['create_entity', 'mutate_entity', 'prepare_revision', 'capture_revision'].includes(fn.proname as string));
    }
    ...
    const denied = [
      ...
      'SET ROLE source_owner', 'SET ROLE m1_admin', 'SET ROLE pg_write_all_data',
      ...
      'SELECT source.next_version(1)', 'SELECT source.prepare_revision()', 'SELECT source.capture_revision()',
      'ALTER FUNCTION source.capture_revision() SECURITY INVOKER',
      'CREATE TABLE source.shadow(id int)', 'CREATE TABLE public.shadow(id int)', 'CREATE TEMP TABLE shadow(id int)',
    ...
    await sqlError(writer, "SELECT source.mutate_entity($1,'upsert','{}')", '22023', [id]);
    await sqlError(writer, "SELECT source.mutate_entity($1,'restore','{}')", '22023', [id]);
    await sqlError(writer, "SELECT source.mutate_entity($1,'delete','{}')", '22023', [id]);
    for (const badPayload of [null, 'null', '[]', '"scalar"']) await sqlError(writer, 'SELECT source.create_entity($1::jsonb)', '23514', [badPayload]);
    await mutateEntity(writer, id, 'delete');
    await sqlError(writer, "SELECT source.mutate_entity($1,'update','{}')", '22023', [id]);
    await sqlError(writer, "SELECT source.mutate_entity($1,'restore',NULL)", '23514', [id]);
    ...
    await sqlError(observer, `DELETE FROM source.entities WHERE entity_id=${id}`, '0A000');
    await sqlError(observer, 'TRUNCATE source.entities CASCADE', '0A000');
    const triggers = (await observer.query("SELECT c.relname, t.tgname, t.tgenabled FROM pg_trigger t JOIN pg_class c ON c.oid=t.tgrelid WHERE c.relnamespace='source'::regnamespace AND NOT t.tgisinternal ORDER BY t.tgname")).rows;
    assert.equal(triggers.length, 6);
    for (const t of triggers) assert.equal(t.tgenabled, 'O');
    evidence('T06', { identity, ownerRole, owners, functions, deniedOperations: denied.length, final, triggers });
  });
});

test('T07 known allocation-order risk: B commits first, late A remains queryable by identity', { timeout: 20_000 }, async () => {
  ...
  try {
    ...
    assert.ok(BigInt(aEvent.allocation_id as string) < BigInt(bEvent.allocation_id as string));
    await b.query('COMMIT');
    ...
    evidence('T07-B-visible-A-uncommitted', { aEvent, bEvent, visibleA: await revisions(aRow.entity_id), visibleB: await revisions(bRow.entity_id) });
    await a.query('COMMIT');
    ...
    const unsafe = (await observer.query('SELECT entity_id::text, entity_version::text FROM source.outbox WHERE entity_id IN ($1,$2) AND allocation_id > $3', [aRow.entity_id, bRow.entity_id, bEvent.allocation_id])).rows;
    assert.deepEqual(unsafe, []);
    evidence('T07', { aEvent, bEvent, committedA: await revisions(aRow.entity_id), committedB: await revisions(bRow.entity_id), unsafeGreaterThanB: unsafe, conclusion: 'allocation order is not commit order; no incremental completeness claim' });
  } finally {
    await Promise.allSettled([a.query('ROLLBACK'), b.query('ROLLBACK')]);
    await Promise.all([a.end(), b.end()]);
  }
...
test('T10 source tests leave no runtime database sessions or instrumentation', async () => {
  ...
  const instrumentation = (await observer.query("SELECT proname FROM pg_proc WHERE pronamespace='source'::regnamespace AND proname LIKE 'test_%'")).rows;
  assert.deepEqual(instrumentation, []);
  evidence('T10-source-cleanup', { sessions, instrumentation, note: 'service restart, owned-resource cleanup, and a fresh full run are checked by the orchestration and final acceptance' });
});
