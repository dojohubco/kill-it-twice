...
export async function oracle(
  ...
  p: pg.Client,
  es: EsTransport,
  expectedRejections: readonly ExpectedRejection[],
) {
  const rejected = new Map(expectedRejections.map((r) => [r.eventId, r]));
  assert.equal(
    rejected.size,
    expectedRejections.length,
    'Duplicate verifier rejection declaration',
  );
  const diagnostics = (
    await p.query<
      Record<string, unknown>
    >(`SELECT d.*, a.event_id attempt_event,a.destination_id attempt_destination,
    a.outcome attempt_outcome,a.context attempt_context,a.finished_at::text finished_at,i.attempt_id::text terminal_attempt
    FROM pipeline.es_dead_letters d JOIN pipeline.es_attempts a USING(attempt_id)
    JOIN pipeline.delivery_intents i ON i.event_id=d.event_id AND i.destination_id=d.destination_id ORDER BY d.event_id`)
  ).rows;
  assert.deepEqual(
    diagnostics.map((r) => r['event_id']).sort(),
    [...rejected.keys()].sort(),
    'Actual rejected identities must exactly equal the pre-delivery fixture declarations',
  );
  const expectedHistory = new Map<string, ExpectedReceiver>();
  const revisions = (
    await s.query<{
      event_id: string;
      body: string;
    ...
    w.state,w.acknowledged_hash, CASE WHEN o.is_deleted THEN '{}' ELSE (SELECT COALESCE(jsonb_object_agg(key,value),'{}')::text FROM jsonb_each(o.payload) WHERE key IN ('name','country','loyalty_points')) END search_fields FROM source.outbox o LEFT JOIN source.capture_work w USING(source_epoch,entity_id,entity_version) ORDER BY event_id`)
  ).rows;
  const ledgerRows = (
    await p.query<{
      event_id: string;
  ...
  for (const revision of revisions) {
    ...
    const rejectedRevision = rejected.get(revision.event_id);
    if (rejectedRevision) {
      assert.equal(fields['payload_json'], rejectedRevision.payloadText);
      assert.equal(
        createHash('sha256').update(rejectedRevision.payloadText).digest('hex'),
        rejectedRevision.payloadSha256,
      );
    }
    checkExpectedOutcome(
      revision.event_id,
      intents[0]?.state ?? '',
      rejectedRevision,
      diagnostics.find((d) => d['event_id'] === revision.event_id),
    );
    const entity = fields['entity_id'],
      epoch = fields['source_epoch'],
      version = fields['entity_version'];
    assert.ok(
      typeof entity === 'string' &&
        typeof epoch === 'string' &&
        typeof version === 'string',
    );
    expectedHistory.set(revision.event_id, {
      eventId: revision.event_id,
      documentId: `${epoch}:${entity}`,
      version,
      source: {
        ...
        content_sha256: hash,
        search_fields: parse(revision.search_fields),
      },
    });
 ...
 CASE WHEN is_deleted THEN '{}' ELSE (SELECT COALESCE(jsonb_object_agg(key,value),'{}')::text FROM jsonb_each(payload) WHERE key IN ('name','country','loyalty_points')) END AS search_fields FROM source.entities ORDER BY entity_id`)
  ...
  for (const r of rows) {
    ...
    const documentId = `${r.source_epoch}:${r.entity_id}`;
    const current = expectedHistory.get(id);
    assert.ok(current, 'Latest source revision absent from immutable history');
    ...
    assert.equal(
      current.source['canonical_body_json'],
      currentBody,
      'Current row and immutable revision disagree',
    );
    assert.deepEqual(current.source['search_fields'], parse(r.search_fields));
    const entityHistory = [...expectedHistory.values()]
      .filter((e) => e.documentId === documentId)
      .sort((a, b) => (BigInt(a.version) < BigInt(b.version) ? 1 : -1));
    assert.equal(
      entityHistory[0]?.eventId,
      id,
      'Current source revision is not the last historical revision',
    );
    const admissible = entityHistory.find((e) => !rejected.has(e.eventId));
    const reply = object(
      await es.request(
        'POST',
        `/${required('ES_INDEX')}/_mget`,
        JSON.stringify({ ids: [documentId] }),
      ),
    );
    const docs = reply['docs'];
    assert.ok(Array.isArray(docs) && docs.length === 1);
    const actual = object(docs[0]);
    checkReceiver(documentId, admissible, actual);
    if (admissible) expectedIds.push(documentId);
    const unresolved = admissible?.eventId !== id;
    evidenceRows.push({
      id,
      sourceRevision: r.entity_version,
      expectedReceiverRevision: admissible?.version ?? null,
      expectedReceiverEvent: admissible?.eventId ?? null,
      unresolved,
      convergence: unresolved ? 'EXPECTED DEGRADED' : 'CONVERGED',
      version: admissible ? version(actual['_version']) : null,
      source: actual['_source'] ?? null,
      expectedRejectedRevisions: entityHistory
        .filter((e) => rejected.has(e.eventId))
        .map((e) => e.eventId),
      delivery: (await deliveries(p, [id]))[0],
    });
  }
  const search = object(
    await es.request(
  ...
  checkSearchIdentities(expectedIds, hits);
  assert.equal(
    object(search['hits'])['total'] &&
      object(object(search['hits'])['total'])['relation'],
    'eq',
  );
  assert.equal(
    String(object(object(search['hits'])['total'])['value']),
    String(expectedIds.length),
  );
  evidence('ES14-reconciliation', {
    history,
    expectedRejections,
    overall: evidenceRows.some((r) => r.unresolved)
      ? 'EXPECTED DEGRADED'
      : 'CONVERGED',
    rows: evidenceRows,
