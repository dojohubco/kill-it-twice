...
import { esConfig, ledgerConfig } from './es.ts';
...
export function launchEs(
  ...
  const child = fork(new URL('./es-child.ts', import.meta.url), [], {
    execArgv: [],
  ...
  t.after(async () => {
    ...
    await waitFor(() => closed, Boolean, 'ES child cleanup', 5000);
    evidence(`${label}-cleanup`, { pid, exit, closed, overflow });
  });
  child.send({
    ledger: ledgerConfig(label),
    receiver: esConfig(false, proxy),
    boundary,
    leaseMs,
  ...
  return {
    ...
    async barrier() {
      ...
      assert.equal(msg['type'], 'es-barrier');
    ...
    async finish(mode: 'kill' | 'release' | 'run' | 'term' | 'stale') {
      ...
      await waitFor(() => closed, Boolean, 'ES child exit', 10000);
      assert.deepEqual(
        exit,
        mode === 'kill'
