...
if (!process.send)
  throw new Error('Writer child requires its private parent IPC channel');
...
const close = mock.method(
  ...
  async function (this: pg.Client) {
    try {
      if (
        start?.barrier === 'source.after_commit.before_caller_success' &&
        telemetry &&
        !orphaned
      )
        await barrier();
...
try {
  ...
  assert.ok(
    'barrier' in message &&
      (message.barrier === 'source.after_mutation.before_commit' ||
        message.barrier === 'source.after_commit.before_caller_success'),
  );
  assert.ok(
    'payloadJson' in message && typeof message.payloadJson === 'string',
  );
  assert.ok('port' in message && typeof message.port === 'number');
  assert.ok('password' in message && typeof message.password === 'string');
  assert.ok(
    'applicationName' in message && typeof message.applicationName === 'string',
  );
  start = {
    type: 'start',
    runId: message.runId,
  ...
  const entity = await owner.transaction(async (tx) => {
    ...
    telemetry = {
      ...
      backendPid: inspection.session.pid,
      transactionId: inspection.session.xid,
      applicationName: config.applicationName,
      sessionUser: inspection.session.session_user,
      effectiveUser: inspection.session.current_user,
      entity: row,
      outbox: inspection.outbox,
    };
    if (config.barrier === 'source.after_mutation.before_commit')
      await barrier();
    return row;
  });
  if (orphaned) throw new Error('Unexpected parent loss');
  process.stdout.write(
    JSON.stringify({ type: 'caller-success', entity }) + '\n',
  );
