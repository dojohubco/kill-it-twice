...
COLLECTIONS = ('baselines', 'source', 'mutations', 'commands', 'work', 'pipeline', 'consumer', 'totals', 'projection', 'receiver', 'failures')
...
def inspect(directory, expected_count, journal, database, rejected_commands):
    ...
    for name in COLLECTIONS:
        with (directory / f'{name}.jsonl').open() as stream:
            for line in stream:
                ...
                row = parse(line)
                key = row['event_id'] + ':' + row['attempt_id'] if name == 'failures' else row['document_id'] if name == 'receiver' else row['event_id'] if name in ('pipeline', 'consumer') else row['command_id'] if name == 'commands' else row['cursor']
    ...
    connection.execute('CREATE TABLE ordinals (ordinal INTEGER PRIMARY KEY)')
    connection.execute('CREATE TABLE expected_receiver (entity TEXT PRIMARY KEY,event TEXT NOT NULL)')
    connection.execute('CREATE INDEX history_entity ON history(entity,kind)')
    ...
    for row in each('baselines'):
        ...
        assert 1 <= ordinal <= expected_count
        connection.execute('INSERT INTO ordinals VALUES(?)',(ordinal,))
        ...
        connection.execute('INSERT INTO expected_receiver VALUES(?,?)', (row['entity_id'], event_id(row)))
    mutation_by_id = {}
    ...
    assert len(rejected_commands) == len(set(rejected_commands))
    for request in requests:
        ...
        if request['command_id'] in rejected_commands:
            assert changed and wanted['loyalty_points']=='not-a-number'
            expected_rejections.add(key)
        else:
            connection.execute('INSERT INTO expected_receiver VALUES(?,?) ON CONFLICT(entity) DO UPDATE SET event=excluded.event',(entity,key))
    ...
    assert count('source') == count('projection') == count('totals') == entities
    assert count('receiver') == connection.execute('SELECT count(*) FROM expected_receiver').fetchone()[0]
    assert len(expected_rejections)==len(rejected_commands)
    failures=list(each('failures'))
    assert len(failures)==len(expected_rejections)
    assert {r['event_id'] for r in failures}==expected_rejections
    assert all(r['error_class']=='mapping' and r['outcome']=='mapping' and r['state']=='dead_letter' and r['current'] is True and ('document_parsing_exception' in r['context'] or 'mapper_parsing_exception' in r['context']) for r in failures)
    ...
    for key, entity, version, kind, encoded, digest in connection.execute('SELECT * FROM history'):
        ...
        assert event['es_state'] == ('dead_letter' if key in expected_rejections else 'satisfied')
        assert event['rabbit_state'] == 'satisfied' and event['consumer_state'] == 'processed'
    ...
    for entity, key in connection.execute('SELECT * FROM expected'):
        ...
        accepted=connection.execute('SELECT event FROM expected_receiver WHERE entity=?',(entity,)).fetchone()
        if accepted is None:
            assert connection.execute("SELECT count(*) FROM records WHERE collection='receiver' AND key=?",(f'{epoch}:{entity}',)).fetchone()[0]==0
            continue
        h=connection.execute('SELECT body,hash FROM history WHERE event=?',(accepted[0],)).fetchone()
        value=parse(h[0])
    ...
    connection.commit()
    connection.close()
    return dict(status='PASS', scope='Independent frozen-fixture comparison; execution scale is reported separately',
        source_epoch=epoch, baselines=expected_count, entities=entities, historical_events=total,
        mutation_effects=len(expected_mutations), expected_rejected_revisions=len(expected_rejections), oracle='verifier-owned commands and seed recipe, exact revisions/bytes/relations/values')
...
if __name__ == '__main__':
    ...
    arguments.add_argument('--rejections', required=True, type=Path)
    args = arguments.parse_args()
    assert 257 <= args.count <= 2000000, 'Explicit finite fixture bound'
    print(json.dumps(inspect(args.directory, args.count, args.journal, args.directory / 'oracle.sqlite', json.loads(args.rejections.read_text())), indent=2))
