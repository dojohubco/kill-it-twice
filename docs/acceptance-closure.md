...
## Authorized polling and public handoff continuation, 2026-09-25

The user subsequently authorized durable UI polling edits and publication to the existing public `dojohubco/kill-it-twice` repository, with no careers application. ADR 016 records the polling decision. Current tested `1f2f3f2d544a70b1e63d40d3a0f27d62f4249685` passed strict 1024 and full local 1M acceptance, retained R01–R08, exact oracle, five genuine negatives and both cleanups on September 25. See [current report](evidence/Final-submission-2026-09-25.md). The older scope statements above remain historical; the later publication authorization applies. Original failures, server/demo state and old ZIPs are preserved. This closure changes documentation/evidence only; normal fast-forward publication and hosted CI are recorded separately. No optimization, new shared-server workload or optional 2M is required.
