...
## Later final-workload evidence

The corrected 262144-row sequence at `93eac82` completed and reconciled exactly; see [its inspected evidence](evidence/Observation-terminal.md). The later million-row final run at `f0f98d8` failed on a different operational query: the pipeline snapshot itself exceeded 2500 ms. Its worker logs recorded terminal completion, but its independent final oracle never ran. The scoped two-index correction and separately measured 8000-ms operational backfill-read budget are recorded in [closure development evidence](evidence/Acceptance-closure-development.md#million-row-operational-snapshot-timeout). Full terminal proof remains mandatory and unchanged. The earlier investigation and its original budget decision above are historical, not proof that every later workload passed.
