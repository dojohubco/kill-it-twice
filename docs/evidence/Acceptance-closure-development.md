...
## Verifier overhead and accidental demo removal (2026-09-24)

The sequential-setup diagnostic `kit-final-20260924091530-44e5b69a` failed G1 before SIGKILL: two one-off inspector reads took 17.09 and 16.90 seconds across a 24-second private pre-COMMIT barrier. The checkpoint comparison passed, but the held transaction was no longer present for the second observation. This is a verifier failure, not evidence of failed application rollback. G2-G5 did not run; cleanup passed. The diagnostic setup is not cold-start acceptance.

A real isolated PostgreSQL SELECT 1 comparison (`kit-final-20260924093259-c804af2e`) returned the exact expected row in all six reads. One-off container reads took 5.795, 4.799 and 4.402 seconds; fresh Node processes inside a resident inspector took 2.199, 1.600 and 2.100 seconds. Its original manifest remains FAIL because cleanup left the tool-profile inspector. An earlier helper also failed an immediate PostgreSQL readiness probe; both failures are retained. Separate exact-resource cleanup recovery eventually passed. These measurements justify removing container creation from fault-boundary observations, not changing the database or fault assertions.

The verifier now opts into one owned resident inspector after normal cold initialization, executes a fresh Node process for every JSON read, and removes it with the other validated run-owned resource IDs during cleanup. Worker/receiver restart targets the validated existing container ID, avoiding Compose restarting completed provision/realm/initializer dependencies. The 24-second fault barrier, 30-second production idle-transaction limit, SQL/lease limits and all exact assertions remain unchanged. The real PostgreSQL lifecycle regression `kit-final-20260924094951-ba9d65d4` passed: both exact reads, fresh Node processes, the same inspector container across restart, unchanged provisioning StartedAt, cleanup PASS and unchanged recreated-demo container IDs. It ran on the developmental tree with the existing 79e image; it is not clean final acceptance. `make quality` passed with 88 tests and no skips; an initial documentation-formatting failure is retained.

During manual recovery of the timing diagnostic, the assistant ran Compose from an inaccessible inherited working directory after `runuser`, without setting subprocess cwd to the repository. Despite the explicitly supplied diagnostic project argument, the recorded cleanup output shows deletion of `kit-server-dev` containers and volumes. This violated the requirement to preserve the retained demo. Original demo data is not recovered; no backup was identified in the inspected setup artifacts. The exact fallback mechanism is unproven: read-only config fails from the inherited directory, while explicit repository cwd resolves the intended project. The incident and original deletion log are retained under `artifacts/server-restart/demo-cleanup-incident-20260924/` and the timing run's `cleanup-recovery.stderr.log`. LogisticsHub containers remained running in the subsequent snapshot; that is not a complete application health audit.

Demo functionality was recreated at 09:49:49 UTC using its retained original image with a NEW installation and 1024 baseline records. The API reported healthy dependencies, completed backfill and 1024 processed records with zero effects/quarantine. New source epoch: `95af5a3e-1e25-4e5e-b065-548721a3d73d`; new pipeline: `598e4e23-c43e-477d-9fb1-ab293cb1b7ef`. This is not restoration of the deleted identities/history and must never be presented as such. Before future Compose operations, Runtime validates the resolved project and owned volume/network names in addition to its existing explicit repository cwd. Runtime cleanup now avoids Compose down altogether: within its aggregate 180-second budget it lists the exact project resources, validates their labels, stops/removes those container IDs, then removes only their labeled volumes and empty networks. Manual recovery likewise validates exact resource labels and uses explicit cwd. Four refusal tests cover demo names, foreign container labels, wrong resolved projects and external volumes. The user was informed of the incident. No acceptance or publication claim follows from this recovery.

Failure diagnostics also retain bounded private Elasticsearch, Toxiproxy and initializer logs/state before cleanup, prioritizing these services for prerequisite/G3 failures within the existing aggregate 60-second capture budget. These private logs are excluded from the public worker-log copy. Capture failures remain explicit. Clean full fault acceptance is still pending.
