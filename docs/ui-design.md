...
## Operator copy cleanup — 2026-09-26

The six routes use short task descriptions, contextual errors and action consequences. Implementation commentary about fixtures, backend contracts and verification belongs in engineering documentation. Status freshness, precise identifiers, saved-versus-applied settings and ambiguous-action recovery remain explicit where they affect an operator decision. The existing dark tokens, typography, layout and motion remain in use; removing the sidebar environment card requires no replacement decoration. The API schema link downloads the real JSON document instead of opening a raw-data tab.

| Before                                        | After                                                 | Why                                          |
| --------------------------------------------- | ----------------------------------------------------- | -------------------------------------------- |
| Environment and receiver-outcome slogans      | Sidebar navigation and schema download                | Remove text that does not help an action     |
| Repeated implementation and proof disclaimers | Short task descriptions and contextual state messages | Keep the operator focused on current work    |
| Raw schema opened in a new tab                | Explicit JSON download                                | Match the document format to the link action |

`make verify-ui` exited 0 on this implementation: quality checks, 90 unit tests and all 12 required browser cases passed, with no skips. The unchanged stale-state and accepted-request assertions now expect the shorter visible messages; the state, identity, retry and completion checks remain in place. Real local rendering showed all five dependencies healthy/fresh, 257 staged/search/broker/consumer records, completed backfill and zero pending failures. The schema endpoint returned OpenAPI 3.0.0 with 19 paths; its sidebar link now downloads that document. The local UI service alone was replaced; all other 15 service IDs, images and states and all 16 mount sets were unchanged. The README screenshot is that actual retained demo, while the September 21 report keeps its dated original image.
