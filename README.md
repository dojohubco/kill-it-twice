...
The authorized public source target is [dojohubco/kill-it-twice](https://github.com/dojohubco/kill-it-twice). The local acceptance report identifies the exact tested code; the publication receipt and [GitHub Actions](https://github.com/dojohubco/kill-it-twice/actions) separately identify the published revision and hosted checks. A local PASS alone is not a hosted-CI result. No careers application is included. The [acceptance matrix](docs/acceptance-matrix.md) identifies any remaining blocker and the exact tested versus later documentation identities.
