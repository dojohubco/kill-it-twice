...
Source development check: typecheck/lint and 5 unit tests passed. The first source run (`artifacts/m1/m1-20260916160939467-d8ba6031/`) passed 8/9 integration tests. T06 expected privilege error `42501` when assigning a generated identity, but PostgreSQL rejects this earlier with `428C9` (generated-always). This commit retains that incorrect test expectation for a focused correction. It does not indicate that the protected write succeeded. SQL function/grant details were recorded in `docs/milestones/M1.md` before migration execution.
