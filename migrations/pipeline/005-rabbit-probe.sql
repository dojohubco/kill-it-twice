-- Forward M4 correction: do not lease a recovery probe without eligible work.
-- Replaces only the function body; preserves all rows, identities and grants.
...
BEGIN
 ...
 END LOOP;
END $$;
REVOKE ALL ON FUNCTION pipeline.rabbit_claim(uuid,bigint,uuid,integer,integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION pipeline.rabbit_claim(uuid,bigint,uuid,integer,integer) TO pipeline_rabbit;
RESET ROLE;
